...
{
    public partial class OrderDetailList : Form
    {
        ...
        private void BtnEXCEL_Click(object sender, EventArgs e)
        {
            ...
            string strsql = "select 订单编号 as  OrderCade,标题 as Title,购买数量 as BuyNO,商家编码 as BarCode,套餐信息 as AttriBute,0 as Pid,0 as ColourID,0 as SDID,'OD"
                        + DTdatetime.Value.ToString("yyyyMMdd") + getDate.uppacking("STR_OrderDetailList", DTdatetime.Value.ToString("yyyyMMdd")) + "' as Cade,'"
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = sqlcon.getcon("");
            DataSet ds = new DataSet();
            SqlDataAdapter sqlDaper = new SqlDataAdapter("select * from STR_OrderDetailList where " + sqlselect, conn);
            conn.Open();
            sqlDaper.Fill(ds);
            conn.Close();
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("delete from STR_OrderDetailList where " + sqlselect, conn);
                    cmd.ExecuteNonQuery();
                    conn.Close();
                }
                else
            ...
            try
            {
                ...
                //    else
                //    {
                //        strsql += "insert into STR_OrderDetailList (" +
                //            "SHOPid,Cade,CadeDate,OrderCade,Title,buyNO,BarCode,Attribute,username) values('"
                //        + CmdShop.SelectedValue.ToString() + "','OD"
                //        + DTdatetime.Value.ToString("yyyyMMdd") + getDate.uppacking("STR_OrderDetailList", DTdatetime.Value.ToString("yyyyMMdd")) + "','"
                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    ...
                    {
                        ////服务器上目标表的名称     
                        sbc.DestinationTableName = "STR_OrderDetailList";
        ...
        private void BTNDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                strsql = "delete from STR_OrderDetailList " + strsql;
                conn.Open();
                SqlCommand cmd = new SqlCommand(strsql, conn);
