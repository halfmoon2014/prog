...
{
	public partial class sqb_bweb_rount_select : System.Web.UI.Page
	{
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["mode"] == "getdatagrid")
            {
                ...
                String zzjg = Request.QueryString["zuzhijg"].ToString();
        ...
        private void getdatagrid(String starttime, String endtime,String zzjg, String linkman, String client_id, String call_mode, String call_type)
        {
            ...
            if (starttime == "" || endtime == "" || zzjg=="")
            {
               // sqlstr = "select TOP " + Convert.ToInt32(Request["rows"].ToString()) + " id,name,username,date,call_type,call_mode,job_content from client_call_select where (id >=  (  SELECT ISNULL(MAX(id),0)  FROM (SELECT TOP (" + Convert.ToInt32(Request["rows"].ToString()) + "*(" + Convert.ToInt32(Request["page"].ToString()) + "-1)+1) id FROM client_call_select ORDER BY id ) A ))";
                sqlstr = "SELECT TOP " + Convert.ToInt32(Request["rows"].ToString()) + " id,name,username,date,call_type,call_mode,job_content FROM (SELECT ROW_NUMBER() OVER (ORDER BY id) AS RowNumber,* FROM client_call_select) A WHERE RowNumber > (" + Convert.ToInt32(Request["rows"].ToString()) + "*(" + Convert.ToInt32(Request["page"].ToString()) + "-1))";
            ...
            else
            {
               ...
               //     "where ("+linkman+" and client_id like '%" + client_id + "%' and call_mode like '%" + call_mode + "%' and call_type like '%" + call_type + "%') and date between '" + starttime + "' and '" + endtime + "' and (id >=  (  SELECT ISNULL(MAX(id),0)  FROM (SELECT TOP (" + Convert.ToInt32(Request["rows"].ToString()) + "*(" + Convert.ToInt32(Request["page"].ToString()) + "-1)+1) id FROM client_call_select ORDER BY id ) A ))";
                ...
                sqlstr = "SELECT TOP " + Convert.ToInt32(Request["rows"].ToString()) + " id,name,username,date,call_type,call_mode,job_content FROM (SELECT ROW_NUMBER() OVER (ORDER BY id) AS RowNumber,* FROM client_call_select  where (" + linkman + " and client_id like '%" + client_id + "%' and call_mode like '%" + call_mode + "%' and call_type like '%" + call_type + "%') and CONVERT(varchar(100), date, 23) between '" + starttime + "' and '" + endtime + "') A WHERE RowNumber > (" + Convert.ToInt32(Request["rows"].ToString()) + "*(" + Convert.ToInt32(Request["page"].ToString()) + "-1))";
                sql = "select count(*) from client_call_select where (" + linkman + " and client_id like '%" + client_id + "%' and call_mode like '%" + call_mode + "%' and call_type like '%" + call_type + "%') and CONVERT(varchar(100), date, 23) between '" + starttime + "' and '" + endtime + "'";
