...
{
    public partial class sqb_bweb_users : System.Web.UI.Page
    {
        ...
        protected void LoadAllRole(int rows, int page)
        {
            ...
            SqlString = "select count(*) from sqb_users_role";
            RoleTable = SqlQuery.GetDataTable(SqlString);
            
            Response.Write("{ \"total\":" + RoleTable.Rows[0][0].ToString() + ",\"rows\":" + Jss.Serialize(RoleJson) + "}");
            Response.End();
        }
        ...
        protected void SaveRole(string savemode, string id, string allow_id, string name, string note)
        {
            ...
            {
                case "add":
                    SqlString = "insert into sqb_users_role (allow_id,name,note) values(" + (string.IsNullOrEmpty(allow_id) ? "0" : allow_id) + ",'" + name + "','" + note + "')";
                    break;
                case "edit":
